        ...
        public struct ToolInfoWrapper
        {
            ...
            private static IntPtr GetHWND(object handle)
                => handle switch
                {
                    ...
                    */
                    IWin32Window window => window.Handle,
                    _ => throw new InvalidOperationException("Unexpected handle type.")
                };


            public unsafe IntPtr SendMessage(/*IHandle*/IWin32Window sender, uint message, BOOL state = BOOL.FALSE)
            {
                ...
                fixed (void* i = &Info)
                {
                    ...
                    IntPtr result = User32.SendMessageW(sender.Handle, message, (IntPtr)state, (IntPtr)i);
